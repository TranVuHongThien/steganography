...
DEBUG = False
console = Console()
headerText = "JT7sMY455cHVL2" #To check 
...
def convertToRGB(img):
	"""
	Converts the image mode to RGB
	"""
	try:
		rgba_image = img
		rgba_image.load()
...
def getPixelCount(img):
	""" 
	Gets the total amount of pixels in the sample image
	"""
...
def main():
	...
	if op==1:
		print("Name of image file (with extension): (example: sample.jpg)")
		img = input(">>")
		...
		password=""
		while 1:
			print("Enter password to encrypt (AES-256)(leave empty if you want no password): ")
			...
			if(password!=confirm_password):
				print("[red]Passwords don't match. Please try again. [/red]")
			else:
				break
		...
		print("Name of image file (with extension): (example: sample-encoded.png) ")
		img = input(">>")
		if(not(path.exists(img))):
			raise Exception("Image not found!")

		print("Enter password to decrypt (leave empty if no password): ")
		password = getpass.getpass(">>")
